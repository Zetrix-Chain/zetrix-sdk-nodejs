...
const { keypair, signature } = require('zetrix-encryption-nodejs');
const errors = require('../exception');


const proto = exports;

const commonOperationPayCoin = require("./operation/payCoin");
const commonOperationAccountSetPrivilege = require("./operation/accountSetPrivilege");
const commonOperationActivateAccount = require("./operation/activateAccount");
const commonOperationContractCreate = require("./operation/contractCreate");
const commonOperationContractInvokeByGas = require("./operation/contractInvokeByGas");

const cryptoProtobufBundle = require("../crypto/protobuf/bundle.json");
...
proto._getDefaultValue = function* () {
  try {
    ...
    const feeLimit = long.fromValue(1000).mul(gasPrice); //feeLimit = gasPrice * tx_size
    return {
      gasPrice,
      feeLimit,
    }
...
proto._responseErrorMsg = function(code, message) {
  if (!code && !message) {
    throw new Error('require code and message');
  }
...
proto._buildOperation = function(type, data) {
  try {
    if (type === "payCoin") {
      return commonOperationPayCoin(data);
    } else if (type === "accountSetPrivilege") {
      return commonOperationAccountSetPrivilege(data);
    } else if (type === "activateAccount") {
      return commonOperationActivateAccount(data);
    } else if (type === "contractCreate") {
      return commonOperationContractCreate(data);
    } else if (type === "contractInvokeByGas") {
      return commonOperationContractInvokeByGas(data);
    }
...
proto._buildBlob = function(args) {
  try {
    ...
    const root = protobuf.Root.fromJSON(cryptoProtobufBundle);
    ...
    const payload = {
      ...
      feeLimit: long.fromValue(feeLimit), //feeLimit = gasPrice * tx_size
...
proto._isAvailableZtx = function (str) {
